      ...
      <div ref={containerRef} className="relative max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          style={{
            height: threadHeight,
            background:
              'linear-gradient(to bottom, rgba(251, 146, 60,0) 0%, #fb923c 18%, #fb923c 82%, rgba(251, 146, 60,0) 100%)',
                ...
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 top-1 z-10 flex items-center justify-center">
                  <div
                    className={`relative h-3 w-3 rotate-45 ring-[6px] ring-bg ${
                      item.type === 'current' ? 'bg-peach shadow-[0_0_18px_rgba(251, 146, 60,0.7)]' : 'bg-ink'
                    }`}
                  >
                    {item.type === 'current' && (
                      <span className="absolute inset-0 animate-ping bg-peach" />
                    )}
                  </div>
                </div>
    ...
    >
      <div
        className={`relative p-6 md:p-8 border ${
          isCurrent
            ? 'border-peach/50 bg-peach-dim'
            : 'border-border-soft bg-surface/40 hover:border-peach/40'
      ...
      >
        ...
        >
          <span className={isCurrent ? 'text-peach' : 'text-muted'}>{item.year}</span>
        ...
        <span
          ...
          } h-full w-px bg-peach scale-y-0 group-hover:scale-y-100 transition-transform duration-500 origin-top`}
        />
      </div>
    </motion.div>
  );
