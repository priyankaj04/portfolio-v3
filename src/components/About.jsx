      ...
      <div className="relative grid grid-cols-12 gap-6 md:gap-14">
        ...
        >
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted mb-6">
            <span className="text-peach">//</span> bio.md
          </div>
          <p className="font-sans text-xl md:text-3xl leading-[1.5] text-ink/85">
            Passionate, curious developer constantly looking for problems to solve. I work at the intersection of <span className="text-peach">full stack engineering</span> and <span className="text-peach">AI</span> — building systems that scale, agents that think, and products that <span className="text-peach">matter</span>.
          </p>
        ...
        >
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted mb-8">
            <span className="text-peach">//</span> current_focus
          </div>
          <ul className="space-y-3">
            {about.currentFocus.map((item, i) => (
              <motion.li
                ...
                className="group flex items-start gap-4 py-4 border-b border-border-soft hover:border-peach/40 transition-colors"
              >
                <span className="font-mono text-xs text-muted pt-2">0{i + 1}</span>
                <div className="flex-1">
                  <span className="font-syne text-xl md:text-2xl text-ink group-hover:text-peach transition-colors">
                    {item}
                  </span>
                </div>
                <span className="font-mono text-peach opacity-0 group-hover:opacity-100 transition-opacity pt-2">→</span>
              </motion.li>
            ))}
          </ul>

          <div className="mt-10 inline-flex items-center gap-3 px-3 py-2 border border-peach/40 bg-peach-dim">
            <span className="pulse-dot relative inline-flex h-2 w-2 bg-peach" />
