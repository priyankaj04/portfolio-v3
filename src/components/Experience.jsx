  ...
  return parts.map((part, i) => {
    if (/^\d+%|\d+x|1 hr\/user\/month$/.test(part)) {
      return (
        <span
          key={i}
          className="font-mono text-peach font-medium border border-peach/40 bg-peach-dim px-1.5 py-0.5 mx-0.5 align-middle text-[0.9em]"
        >
          {part}
        </span>
      );
      ...
      <div className="space-y-6 md:space-y-8">
        {experience.map((exp, i) => (
          <motion.article
            ...
            className="group relative border border-border-soft bg-surface/30 hover:bg-surface/60 hover:border-peach/40 transition-all duration-500"
            data-hover
          >
            {/* Peach edge on hover */}
            <span className="absolute top-0 left-0 h-full w-px bg-peach scale-y-0 group-hover:scale-y-100 transition-transform duration-700 origin-top" />
            ...
            <div className="grid grid-cols-12 gap-6 md:gap-12 p-6 md:p-10">
              ...
              <div className="col-span-12 lg:col-span-5">
                ...
                <h3
                  className="font-display font-semibold text-ink leading-[0.95] mb-3 group-hover:text-peach transition-colors"
                  style={{ fontSize: 'clamp(34px, 4.6vw, 60px)' }}
                >
                  {exp.company}
                </h3>
                <div className="font-syne text-base md:text-lg text-peach tracking-wide mb-2">
                  {exp.role}
                </div>
              ...
              <div className="col-span-12 lg:col-span-7">
                <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted mb-5">
                  <span className="text-peach">//</span> highlights
                </div>
                <ul className="space-y-4">
                  {exp.highlights.map((h, hi) => (
                    ...
                    >
                      <span className="font-mono text-peach flex-shrink-0 pt-1 text-sm">→</span>
