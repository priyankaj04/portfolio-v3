...
function FeaturedCard({ project }) {
  ...
  return (
    <Wrapper
      ...
      className="group relative col-span-12 lg:col-span-8 lg:row-span-2 border border-border-soft bg-surface/30 hover:bg-surface/60 hover:border-peach/40 hover:shadow-[0_0_60px_rgba(251, 146, 60,0.18)] transition-all duration-500 overflow-hidden"
      data-hover
    >
      ...
      <div className="relative p-6 md:p-10 lg:p-12 h-full flex flex-col">
        <div className="flex items-center justify-between mb-8">
          <span className="font-mono text-[10px] tracking-[0.3em] uppercase text-peach">
            ◼ Featured · 01
          </span>
          <span className="font-mono text-[10px] tracking-[0.2em] uppercase text-muted group-hover:text-peach transition-colors truncate max-w-[60%] text-right">
        ...
        <div className="mt-auto">
          ...
          <div className="flex items-end justify-between gap-4 border-t border-border-soft pt-6">
            <div>
              ...
              <div className="font-syne text-lg md:text-2xl text-peach">{project.impact}</div>
            </div>
            <span className="font-mono text-2xl text-muted group-hover:text-peach group-hover:translate-x-1 transition-all">
              ↗
            </span>
          </div>
        </div>
...
function ProjectCard({ project, index, variant }) {
  ...
  return (
    <Wrapper
      ...
      } border border-border-soft bg-surface/30 hover:bg-surface/60 hover:border-peach/40 hover:-translate-y-1 hover:shadow-[0_0_40px_rgba(251, 146, 60,0.12)] transition-all duration-500`}
      data-hover
    >
      <div className="p-6 md:p-8 h-full flex flex-col">
        <div className="flex items-center justify-between mb-5 gap-3">
          ...
          <span className="font-mono text-[10px] text-muted group-hover:text-peach transition-colors flex items-center gap-1.5 min-w-0">
            {project.url && (
        ...
        <h3 className="font-display font-semibold text-ink text-2xl md:text-3xl leading-tight mb-3 group-hover:text-peach transition-colors">
          {project.name}
        </h3>
        ...
        <div className="mt-auto">
          ...
          <div className="border-t border-border-soft pt-4">
            ...
            <div className="font-syne text-base text-peach leading-snug">{project.impact}</div>
          </div>
        </div>
      </div>
    </Wrapper>
...
function ProjectBgSvg({ className }) {
  return (
    <svg viewBox="0 0 600 600" className={className} preserveAspectRatio="xMidYMid slice">
      ...
      <circle cx="500" cy="100" r="12" fill="#fb923c" />
