...
export default function Navbar() {
  ...
  return (
    ...
    >
      {/* Scroll progress */}
      <motion.div
        className="absolute top-0 left-0 right-0 h-px origin-left bg-peach"
      ...
      <div className="flex items-center justify-between px-6 md:px-12 lg:px-16 py-5">
        ...
        <a href="#hero" className="group flex items-center gap-3" aria-label="Home">
          <span className="relative inline-block h-3 w-3 bg-peach transition-transform duration-500 group-hover:rotate-45" />
          <span className="font-mono text-[11px] tracking-[0.32em] uppercase text-ink">
            Priyanka<span className="text-peach">.</span>J
          </span>
        </a>
            ...
            return (
              <a
                ...
                className={`group relative px-3 py-2 font-mono text-[11px] tracking-[0.28em] uppercase transition-colors ${
                  isActive ? 'text-peach' : 'text-muted hover:text-ink'
                }`}
              >
                <span className="opacity-50 mr-1">{item.n}</span>
                {item.label}
                <span
                  className={`absolute left-3 right-3 -bottom-px h-px transition-transform duration-500 origin-left bg-peach ${
        ...
        <a
          href="#contact"
          className="group hidden sm:flex items-center gap-2 border border-border-soft hover:border-peach/60 px-3 py-2 transition-colors"
        >
          <span className="pulse-dot inline-block h-2 w-2 bg-emerald-400" />
          <span className="font-mono text-[10px] tracking-[0.3em] uppercase text-ink group-hover:text-peach transition-colors">
