  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled || !onHome ? 'backdrop-blur-md bg-bg/70 border-b border-border-soft' : 'bg-transparent'
      }`}
    >
      {/* Scroll progress */}
      <motion.div
      ...
      <div className="flex items-center justify-between px-6 md:px-12 lg:px-16 py-5">
        ...
        {/* Nav links — anchor scroll on home, full nav to home anchor on project pages */}
        <nav className="hidden md:flex items-center gap-1">
          {onHome ? (
            ...
            <Link
              to="/#timeline"
              className="group inline-flex items-center gap-2 px-3 py-2 font-mono text-[11px] tracking-[0.28em] uppercase text-muted hover:text-orange transition-colors"
            >
