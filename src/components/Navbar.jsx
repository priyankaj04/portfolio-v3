...
export default function Navbar() {
  ...
  return (
    ...
    >
      {/* Scroll progress */}
      <motion.div
        className="absolute top-0 left-0 right-0 h-px origin-left bg-emerald"
      ...
      <div className="flex items-center justify-between px-6 md:px-12 lg:px-16 py-5">
        ...
        <a href="#hero" className="group flex items-center gap-3" aria-label="Home">
          <span className="relative inline-block h-3 w-3 bg-emerald transition-transform duration-500 group-hover:rotate-45" />
          <span className="font-mono text-[11px] tracking-[0.32em] uppercase text-ink">
            Priyanka<span className="text-emerald">.</span>J
          </span>
        </a>
          ...
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.label}
                <span
                  className={`absolute left-3 right-3 -bottom-px h-px transition-transform duration-500 origin-left bg-emerald ${
        ...
        <a
          href="#contact"
          className="group hidden sm:flex items-center gap-2 border border-border-soft hover:border-emerald/60 px-3 py-2 transition-colors"
        >
          <span className="pulse-dot inline-block h-2 w-2 bg-emerald-400" />
          <span className="font-mono text-[10px] tracking-[0.3em] uppercase text-ink group-hover:text-emerald transition-colors">
